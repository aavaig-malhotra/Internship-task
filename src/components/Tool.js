...
export default function Header({
    ...
    const onTranslate = useCallback(() => {
        ...
        setEnglishSubtitle(formatSub(subtitle));
    ...
    const handleClickYT = async () => {
        let url = prompt('Enter a Youtube Video URL here to get Youtube Video and Its Captions on the Screen');
        ...
        const completeURL = `${baseURL}?url=${url}`;
        ...
        if (data.subtitles) {
            const ext = data.subtitles.split('fmt=')[1];

            console.log(ext);

            if (['ass', 'vtt', 'srt', 'json'].includes(ext)) {
                url2sub(data.subtitles)
                    .then((res) => {
                        clearSubs();
                        setSubtitle(res);
                    })
